...
public class ECGOSCMessageReceiver : MonoBehaviour
{
	...
	void Update ()
	{
		...
		{
			dataReceived = false;
            ShowMessage(OscPacket.FromByteArray(receivedEndPoint, bytePacket));
    ...
    private void ShowMessage(OscPacket packet)
    {
        Debug.Log(((OscMessage)packet).Address);
    }
